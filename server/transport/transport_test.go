...
func Test_GameTranslates(t *testing.T) {
	...
	res := GameToGameResponse(game)

	assert.Len(t, res.Board, 2)
}
...
func Test_GameWithBarriers(t *testing.T) {
	...
	res := GameToGameResponse(game)
	addedBarriers := 3
	assert.Len(t, res.Board, addedBarriers*BarrierSize+2)
}
...
func checkPlayer(t *testing.T, player TPlayer, expected struct{n string; p util.Position}) {
