...
func Test_PlaceBarrierWithNoMoreBarriers(t *testing.T) {
	game := NewTwoPersonGame()
	for i := 0; i < 8; i++ {
		position := u.Position{Row: 14, Col: 1 + 2*i}
		board, err := game.PlaceBarrier(position, PlayerOne)
	...
	board, err := game.PlaceBarrier(u.Position{Row: 0, Col: 1}, PlayerOne)
	assert.Nil(t, err, "No error expected")
	...
	assert.Nil(t, board[illegalPosition], "barrier should not be on board")
	assert.Nil(t, game.Board[illegalPosition])
}
...
................`
	...
	assert.NotNil(t, err)
	assert.Nil(t, newBoard[u.Position{Row: 3, Col : 8}])
	assert.Nil(t, newBoard[u.Position{Row: 3, Col : 9}])
	assert.Nil(t, newBoard[u.Position{Row: 3, Col : 10}])
}
...
func TestAddPlayer(t *testing.T) {
	...
	player, err = game.AddPlayer(uuid.New())
	assert.NoError(t, err)
